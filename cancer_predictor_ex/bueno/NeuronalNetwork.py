...
class NeuronalNetwork:
    def __init__(self, x, y, lr, lambd):
        self.X=x
        self.Y=y
    ...
    def backward_regu(self):
        derror = - (np.divide(self.Y, self.Yh ) - np.divide(1 - self.Y, 1 - self.Yh)) #derivate of error function, Cross-Entropy, not MSE
        s2 = derror * dSigmoid(self.param['N2']) #s2 = derivate of error function * derivate of sigmoid function 
        variaton_W2 = 1./self.param['a1'].shape[1] * np.dot(s2,self.param['a1'].T) - (self.lambd * self.param['W2'])/self.param['a1'].shape[1]#s2 * a1
        variaton_b2 = 1./self.param['a1'].shape[1] * np.dot(s2, np.ones([s2.shape[1],1])) - (self.lambd * self.param['b2'])/self.param['a1'].shape[1] #s2 * array of ones

        ws = np.dot(self.param["W2"].T,s2) #W2 * s2                    
        s1 = ws * self.param['N1']#s1 = W2 * s2 * derivate of the function       
        variaton_W1 = 1./self.X.shape[1] * np.dot(s1,self.X.T) - (self.lambd * self.param['W1'])/self.X.shape[1]#s1 * x
        variaton_b1 = 1./self.X.shape[1] * np.dot(s1, np.ones([s1.shape[1],1])) - (self.lambd * self.param['b1'])/self.X.shape[1]#s1 * array of ones 
    ...
    def gradient_descend_regu(self, epochs):
        np.random.seed(1)                         
        self.nInit()#init weights and bias
        for i in range(0, epochs):#run
            Yh, error = self.forward()
...
#     nn.gradient_descend(50000)#gradient descend algorithm
